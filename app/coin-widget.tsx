...
export default function CoinWidget(){
 ...
 return <View style={[s.page,{backgroundColor:c.background}]}><CoinFront days={days} shape={profile?.coin_shape||"circle"} color={profile?.coin_color||"gold"} numberStyle={profile?.number_style||"classic"} size={240} displayName={profile?.display_name||""} showBorder={profile?.coin_show_border??true} coinPhoto={profile?.coin_photo||undefined} imageOnlyMode={profile?.coin_image_only||false} borderColor={profile?.coin_border_color||undefined} numberColor={profile?.coin_number_color||undefined}/></View>
}
