...
export default function Onboarding(){
 ...
 const router=useRouter(),{setProfile}=useAuth(),colors=useColors();

 const save=async()=>{
  const sobrietyDate=toDatabaseDate(date);
  if(!sobrietyDate){
   Alert.alert("Invalid date","Please enter your V1CE date as MM/DD/YYYY.");
   return;
  }

  const profile={
   email,display_name:name,sobriety_date:sobrietyDate,substances:subs,
   coin_color:"#F5D680",coin_shape:"circle",number_style:"classic",
   coin_show_border:true,coin_border_color:"",coin_number_color:"",
   coin_photo:"",coin_image_only:false,coin_motto:"",avatar_url:"",
   gifted_count:0,is_premium:false,coin_balance:0
  };

  const {data,error}=await supabase.from("SobrietyProfile").insert(profile).select().single();
  if(error){Alert.alert("Error",error.message);return}
  await AsyncStorage.setItem("v1ce_email",email);
  setProfile(data);
  router.replace("/(tabs)");
 };
...
function Button({label,onPress,disabled,colors}:{label:string;onPress:()=>void;disabled?:boolean;colors:any}){
 return <TouchableOpacity disabled={disabled} onPress={onPress} style={[styles.button,{backgroundColor:colors.foreground,opacity:disabled?.3:1}]}><Text style={{color:colors.background,fontWeight:"700",fontSize:18}}>{label}</Text></TouchableOpacity>
}

const styles=StyleSheet.create({
 container:{flex:1,padding:24,paddingTop:80},
 step:{fontWeight:"700",letterSpacing:2,marginBottom:24},
 title:{fontSize:44,fontWeight:"700",lineHeight:48,marginBottom:32},
 input:{borderWidth:2,padding:14,fontSize:18,marginBottom:20},
 button:{height:56,alignItems:"center",justifyContent:"center",marginTop:"auto"},
 wrap:{flexDirection:"row",flexWrap:"wrap",gap:8},
 chip:{paddingHorizontal:14,paddingVertical:10,borderWidth:2}
});
