...
export default function Onboarding(){
 ...
 const save=async()=>{const profile={email,display_name:name,sobriety_date:date,substances:subs,coin_color:"#F5D680",coin_shape:"circle",number_style:"classic",coin_show_border:true,coin_border_color:"",coin_number_color:"",coin_photo:"",coin_image_only:false,coin_motto:"",avatar_url:"",gifted_count:0,is_premium:false,coin_balance:0};const {data,error}=await supabase.from("SobrietyProfile").insert(profile).select().single();if(error){Alert.alert("Error",error.message);return}await AsyncStorage.setItem("v1ce_email",email);setProfile(data);router.replace("/(tabs)")};
 ...
 return <View style={[styles.container,{backgroundColor:colors.background}]}>
  ...
  {step===2&&<><Text style={[styles.title,{color:colors.foreground}]}>WHEN DID{"\n"}YOU START?</Text><TextInput style={[styles.input,{color:colors.foreground,borderColor:colors.foreground}]} placeholder="YYYY-MM-DD" placeholderTextColor={colors.mutedForeground} value={date} onChangeText={setDate}/><Button label="NEXT" disabled={!date} onPress={()=>setStep(3)} colors={colors}/></>}
