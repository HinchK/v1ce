...
export default function Customize(){
 ...
 const[coinPhoto,setCoinPhoto]=useState(profile?.coin_photo||"");const[saving,setSaving]=useState(false);const[uploadingPhoto,setUploadingPhoto]=useState(false);const[customHex,setCustomHex]=useState(/^#[0-9A-Fa-f]{6}$/.test(profile?.coin_color||"")?profile?.coin_color||"":"");
 ...
 const activeColors=resolveCoinColor(color); const pickCoinPhoto=async()=>{if(!profile?.id||uploadingPhoto)return;const permission=await ImagePicker.requestMediaLibraryPermissionsAsync();if(!permission.granted){Alert.alert("V1CE","Photo access is required to choose a coin photo.");return;}const result=await ImagePicker.launchImageLibraryAsync({mediaTypes:["images"],allowsEditing:true,aspect:[1,1],quality:.85});if(result.canceled||!result.assets[0])return;setUploadingPhoto(true);try{const asset=result.assets[0];const response=await fetch(asset.uri);const body=await response.arrayBuffer();const extension=(asset.fileName?.split(".").pop()||asset.mimeType?.split("/").pop()||"jpg").toLowerCase();const contentType=asset.mimeType||"image/jpeg";const path=profile.id+"/coin-"+Date.now()+"."+extension;const{error:uploadError}=await supabase.storage.from("coin-photos").upload(path,body,{contentType,upsert:false});if(uploadError)throw uploadError;const{data}=supabase.storage.from("coin-photos").getPublicUrl(path);setCoinPhoto(data.publicUrl);}catch(error:any){Alert.alert("V1CE",error?.message||"Couldn't upload that photo.");}finally{setUploadingPhoto(false)}};
 const removeCoinPhoto=()=>setCoinPhoto("");
 ...
 return <ScrollView style={{backgroundColor:c.background}} contentContainerStyle={s.page} keyboardShouldPersistTaps="handled">
  ...
  <Section title="SHAPE" c={c}><View style={s.wrap}>{SHAPES.map(item=><TouchableOpacity key={item} onPress={()=>setShape(item)} style={[s.option,{borderColor:shape===item?c.foreground:c.border}]}><Text style={{color:c.foreground,fontSize:11,fontWeight:"800",letterSpacing:1}}>{item.toUpperCase()}</Text></TouchableOpacity>)}</View></Section>

  <Section title="COIN PHOTO" c={c}><Text style={[s.body,{color:c.mutedForeground}]}>UPLOAD A PHOTO TO APPEAR ON YOUR COIN FACE</Text><TouchableOpacity disabled={uploadingPhoto} onPress={pickCoinPhoto} style={[s.photoButton,{borderColor:c.foreground,opacity:uploadingPhoto?.5:1}]}><Text style={{color:c.foreground,fontWeight:"800",letterSpacing:1}}>{uploadingPhoto?"UPLOADING...":coinPhoto?"CHANGE PHOTO":"+ UPLOAD PHOTO"}</Text></TouchableOpacity>{coinPhoto&&<TouchableOpacity disabled={uploadingPhoto} onPress={removeCoinPhoto} style={s.removePhoto}><Text style={{color:c.mutedForeground,fontWeight:"800",letterSpacing:1}}>REMOVE PHOTO</Text></TouchableOpacity>}</Section>
